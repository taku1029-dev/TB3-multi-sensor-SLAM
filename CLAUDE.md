...
- **`rl_navigation_pkg`** — `ament_python` package hosting the RL env, the EKF-input-gate wrapper, and a stand-in release driver. SAC training and `RLNavigation-v1` are still TODO.
    - `rl_navigation_pkg/`
        ...
        - `envs/sac_env.py`: `RLNavigation-v1` (SAC env, ADR-005/008/009/010/011/012). A-1..A-4 + SAC-4 + SAC-5 + SAC-6 complete. 15-dim obs, 2-dim Box action with σ=exp(a·3), 50 ms set_parameters ack deadline (retry-on-timeout per ADR-012:23-30); reset() does Gazebo soft reset (subprocess `gz service .../control` + `.../set_pose` because dynamically-spawned robots don't return via WorldControl.reset) → EKF state reset via `/set_pose` PoseWithCovarianceStamped so /odom matches the teleport → Nav2 random-goal send. step() applies σ via set_parameters and re-samples a new random Nav2 goal whenever the current one terminates; the goal-completion callbacks check future-identity so a cancelled goal's late result doesn't flip `_goal_done` on the next episode's first goal. Terminated on collision/divergence, truncated on timeout/infra-failure. slam_toolbox lifecycle reset helpers are present but unused — SAC-6 bypassed dynamic SLAM for Phase 1 training (rolling-window costmap + identity `map → odom` TF); re-enable for sim-to-real per SAC-7.
        - `agents/sac_trainer.py`: SB3 SAC training loop with `CheckpointCallback` + TensorBoard. `ros2 run rl_navigation_pkg sac_trainer --total-timesteps N --save-dir ...`. First 100k-step run (2026-05-12) finished without converging (see STATUS.md); checkpoints under `sac_runs/phase1_001/`. Next: inference rollout to evaluate the saved policy before deciding on tune-vs-restructure.
...
- **`reset: {model_only: true}` does NOT teleport `ros_gz_sim create`-spawned robots.** Empirically (2026-05-12) the model_only reset only restores SDF-registered initial poses; dynamically-spawned models like our `custom_robot` have no recorded initial pose to revert to. The reset call returns `data: true` but the robot stays wherever the previous episode left it. **Pair the reset with an explicit `gz service /world/<name>/set_pose` call** that names the robot and sets `position: {0,0,0}`. `RLNavigation-v1.reset()` does both.
...
- [`015-odom-silent-grace-window.md`](adr/015-odom-silent-grace-window.md) — `step()` adds one extra `STEP_PERIOD_S` of spin if `/odom` did not advance in the first window (200 ms total before declaring `odom_silent`). Absorbs single-threaded-executor callback-scheduling jitter; preserves ADR-012:51's infra-failure intent. Partially supersedes ADR-012:51.
- [`016-spawn-robot-facing-goal.md`](adr/016-spawn-robot-facing-goal.md) — `reset()` samples the goal first and respawns the robot facing it (yaw = atan2(gy, gx)). Without this, RegulatedPurePursuitController (ADR-013) stalls on large in-place turns from rest and the robot stays at origin for ~65% of goals, producing degenerate stationary-robot training episodes. Partially supersedes ADR-010:73; refines ADR-013.

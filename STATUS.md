...
We are in the **first-inference-evaluated phase** — a 100k-step SAC training run completed 2026-05-12 in ~3 hours wall-clock and produced **mixed results**, and the saved checkpoints have now been evaluated by deterministic inference rollout (2026-05-12). The env, agent, and reward signal are wired (smoke tests pass; `RLNavigation-v1` drives an EKF-aware Nav2 loop with random goals, EKF/Gazebo state reset per episode, identity `map→odom` TF, slam_toolbox bypassed per SAC-6). `ep_rew_mean` moved from -550 (early) to -210 (final), about 2.5× improvement, confirming the env produces a learnable gradient. **But training did not converge** along ADR-010:34-40 criteria: `critic_loss` grew and oscillated heavily (15 → 130+ spikes), `ent_coef` rose 0.11 → 0.20 (= SAC's auto-temperature demanding *more* exploration as training progresses, the opposite of convergence), and `ep_len_mean` oscillates in a U-shape around 130–210 (far below the 600-step truncate that would mean SAC mastered the σ-tuning task). What this means in practice is: the agent partially learns to keep |EKF − GT| smaller than random σ would produce, but it has not found a stable policy.

**Inference verdict (2026-05-12): policy is _usable_, not broken — but not converged.** Five deterministic episodes each were rolled out against `sac_30000_steps.zip`, `sac_60000_steps.zip`, `sac_90000_steps.zip`, and `final.zip` (100k); logs under `./sac_runs/phase1_001/inference/`. Survival-to-timeout rate climbs with training (30k 60% → 60k 80% → **90k 100%** → final 80%) and healthy-run reward is stable around **-70 to -77** across every checkpoint — confirming the policy outputs coherent σ choices for unseen states. Failure modes split between **EKF divergence** (terminated, |EKF−GT| > 2 m: 30k ×2, 60k ×1, final ×1) and **high-error timeouts** (survives 600 steps at reward -1500 to -2900: 90k ×2, final ×1, 30k ×0). 90k is the best checkpoint by survival, but final regresses slightly — consistent with the rising `ent_coef` during training. The trained policy clears the "usable" bar (deterministic σ keeps the robot alive most episodes, with reproducible low-error performance when it doesn't hit a failure mode), but the 1–2-per-5 failure rate means it's not converged. Next: decide between (a) longer training with current setup, (b) hyperparameter / env tweaks per the STATUS hypotheses (σ range, recent-σ in obs, reward shaping, goal-curriculum), or (c) restructure if the failure-mode analysis points there.
...
- **Failure-mode analysis of the 100k-trained policy.** Inference (2026-05-12) cleared the "is the policy usable" gate but exposed two failure modes that are not yet root-caused: (1) **EKF divergence** — robot survives ~80–340 steps then |EKF−GT| exceeds 2 m (e.g. 30k ep3-4: 82–89 steps; 60k ep4: 343 steps; final ep0: 294 steps); (2) **high-error timeout** — full 600-step completion at reward -1500 to -2900 (90k ep0/ep4; final ep0 if not divergence). Open questions: which start-pose / Nav2-goal combinations trigger each, and what σ trajectory does the policy emit in those episodes? A per-step σ + pose-error logger added to `sac_inference.py` would answer both. Without this, choosing between "more training" vs "env/reward redesign" is guesswork.
- **Convergent Phase 1 policy.** Likely directions, ordered by cheapness: tighten σ range from `[0.05, 20]` to `[0.1, 10]`; add recent-σ (last 1–3 actions) and base velocity to obs; reward shaping for divergence proximity (penalty proportional to |EKF−GT|² before the 2 m threshold trips); simplify goal sampler (constant goal for first ~30k steps, random thereafter). Pick after failure-mode analysis above.

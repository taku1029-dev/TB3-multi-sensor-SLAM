...
- **ADR-016 to document the yaw-toward-goal reset.** The reset() change (sample goal first → spawn yaw = atan2(gy, gx) → applied to both gz set_pose and EKF /set_pose) is load-bearing: without it the env trains and evaluates on a degenerate stationary-robot task. The decision touches ADR-013's RPP-selection rationale (which assumed robot orientation at reset would not interact with controller behaviour) and ADR-010:73's reset sequence. Not yet written as an ADR; the change is committed in `24d659d`.

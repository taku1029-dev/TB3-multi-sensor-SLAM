...
- **SAC-5: slam_toolbox lifecycle deactivate fails empirically** — `RLNavigation-v1.reset()` issues `change_state(DEACTIVATE)` but slam_toolbox returns `success=False`, so the subsequent cleanup→configure→activate steps are skipped and the map persists across episodes. Observed cause: `spawn_robot.launch.py`'s `OnStateTransition(goal_state='inactive')` handler is permanent — it re-emits ACTIVATE every time slam_toolbox lands in `inactive`, racing our manual deactivate. Symptom in smoke test: episode 1 needs ~5 goal-resample attempts before Nav2 accepts one (presumably the stale costmap makes the early goals look blocked). Once a goal sticks, motion + reward signal work fine. Repair candidates: (a) wrap the launch handler in a one-shot, (b) replace the lifecycle dance with slam_toolbox's own `clear_changes` / pause+resume services, (c) restart the slam_toolbox process via launch lifecycle. Diagnostic needed: `ros2 lifecycle get /slam_toolbox` during a reset to confirm the racing-activate hypothesis.

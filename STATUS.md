...
- **SB3 SAC training script + `agents/` package contents** — the env is ready (A-1..A-4) but no learner is wired yet. Plan: SAC instantiation + replay buffer + per-phase LR warmup callback (ADR-009:54-57) + episode-mean-reward / plateau-detection convergence checker (ADR-010:34-40) + checkpointing.

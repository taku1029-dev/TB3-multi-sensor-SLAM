...
We are in the **pre-training phase** — env and SB3 SAC trainer are both wired, smoke-tested, and producing the learning signal that previously was flat. The next milestone is the first real SAC training run. The MVP scaffold (`RLNavigation-v0`) coexists with the trained-system env (`RLNavigation-v1`) which now drives an EKF-aware navigation loop: Nav2 chases random goals inside a 10×10 m arena while the agent's σ choices feed `/ekf_input_gate`, the EKF integrates wheel + IMU, and reward is the `|EKF − GT|` divergence. Each episode begins with a clean state — Gazebo teleports the robot to spawn, the EKF is force-reset via `/set_pose`, the rolling-window costmap drops stale obstacles, and Nav2 inherits a fresh goal. Two 100-step random-action smoke episodes (2026-05-12) show identical reward trajectories from -0.001 to -0.137, no goal-resample chatter, and EKF-cov growth visible in Block D before saturation around step 60. `slam_toolbox` is intentionally not launched during training (SAC-6); it will be re-enabled for sim-to-real per SAC-7.
...
- SAC agent — `src/rl_navigation_pkg/rl_navigation_pkg/agents/sac_trainer.py` (training loop with SB3 `SAC` + `CheckpointCallback` + TensorBoard) and `src/rl_navigation_pkg/rl_navigation_pkg/agents/sac_inference.py` (deterministic rollout from a saved checkpoint). Run via `ros2 run rl_navigation_pkg sac_trainer ...` and `ros2 run rl_navigation_pkg sac_inference --checkpoint ...`. SB3 wiring was verified 2026-05-11 (5000 steps, fps≈8, `n_updates` 99→4299, `critic_loss` 0.031→0.002, `ent_coef` 0.97→0.27). **Note 2026-05-11**: that run's `ep_rew_mean` stayed flat at −0.216 because the robot was static — Nav2 had no goal source from inside the env yet. With SAC-4 + SAC-6 wired, the v1 smoke test now produces a non-flat reward signal (verified 2026-05-12 with the same 100-step trajectory in two consecutive episodes growing from -0.001 to -0.137), so a real SAC training run is the next step.
...
- Real Phase 1 SAC training run — env, agent, and the per-episode signal are in place; what's left is to commit to ~3 h of wall-clock (100 k steps) and inspect TensorBoard.

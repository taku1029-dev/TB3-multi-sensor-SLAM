...
**True survival rate (yaw-fix env, 2026-05-12): ~5%.** 4 checkpoints × 5 episodes; logs under `./sac_runs/phase1_001/inference/csv_yawfix/`. **19/20 episodes diverge** in 82-176 steps (mean ~104 steps ≈ 10 s), each crossing the 2 m `|EKF−GT|` threshold around step 42-52 (~5 s) and being terminated at step 82-176. The single surviving episode (90k ep3, goal (3.04, 2.52)) timed out at 600 steps with reward **-9846** and err_max 1.87 m — just under the divergence threshold, achieved by σ_imu μ = 16.19 (near the [0.05, 20] upper bound, effectively switching off IMU fusion). Survival across training is flat: 30k 0%, 60k 0%, 90k 20% (one barely-survived), final 0%. **No checkpoint produced a stable σ-tuning policy.**

**Baseline retrain (phase1_002, 2026-05-12, 100k steps, no env tweaks): no learning.** `ros2 run rl_navigation_pkg sac_trainer --total-timesteps 100000 --save-dir ./sac_runs/phase1_002 --learning-starts 1000 ...`; ~3.8 hours wall-clock at fps≈7, 979 episodes total. **`ep_len_mean` stayed at 89-105 throughout** (initial random-policy baseline ≈ 89; trained policy at 100k ≈ 101) — the policy never learned to survive longer than random. `ep_rew_mean -900 → -1020` (slight degradation). `ent_coef` rose 0.17 → 0.18 (same non-converging pattern as phase1_001). `critic_loss` swung 1k-3k, `actor_loss` ~620. Several `gazebo control reset returned ... Service call timed out` warnings near the end of the run — late-run `gz service` overload, recoverable in env (`reset()` returns partial obs). This negative result settles a question STATUS.md was holding open: **baseline SAC at 100k steps cannot learn σ-tuning in the yaw-fix env**. Next iteration needs env / reward tweaks; pure-`learn(100k)` is not enough.
...
- **Phase 1 retrain that actually learns.** The baseline retrain (`./sac_runs/phase1_002/`, 100k steps, no env tweaks) ran to completion but showed no learning (`ep_len_mean` ~101 throughout, `ep_rew_mean -900 → -1020`). Pure `learn(100k)` in the yaw-fix env is therefore not enough. The next retrain must ship at least one of the cheap env/reward tweaks listed below before launching `phase1_003`.

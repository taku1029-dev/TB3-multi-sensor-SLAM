...
- `SACEnv` Gymnasium env — `src/rl_navigation_pkg/rl_navigation_pkg/envs/sac_env.py`. Id `RLNavigation-v1`, the trained-system env per ADR-005. **A-1..A-4 + SAC-4 complete** — env-side scaffolding for SAC training is done. Observation: 15-dim `Box(-1, 1)` per ADR-008 (lidar quality 4 + camera quality 2 + IMU dyn 6 + EKF cov 3) with per-feature normalisation per ADR-009:35-48. Action: 2-dim `Box(-1, 1)` per ADR-009 (`σ = exp(a · 3)`). Reward: `-sqrt(dx² + dy²) * 10` per ADR-004:39-40 (subtrahend path verified by `reward_probe`). `step()` runs the per-cycle protocol (ADR-011:26-29 / ADR-012:17-30): `set_parameters` to `/ekf_input_gate` with 50 ms ack deadline → `release` Trigger → 100 ms spin → obs/reward construction. Single `set_parameters` timeouts retry the same action up to 10 times (invisible to SAC per ADR-012:23-30); persistent failure → `truncated=True` with `info['truncated_reason']='set_param_unreachable'`. `/odom` not advancing during the spin → `truncated=True` with `'odom_silent'` (ADR-012:51). Collision (lidar < 0.15 m) and EKF divergence (|EKF − GT| > 2 m) → `terminated=True` per ADR-010:21-26. Nav2 random-goal driver per ADR-010:72: each `reset()` cancels the in-flight `NavigateToPose` goal and sends a new uniform-random one (rejection-sampled away from obstacles + spawn within `[-4, 4]^2`); `step()` re-samples mid-episode the moment Nav2 reports goal completion so the robot stays moving for the full 60-s window. `reset()` runs the full ADR-010:73 sequence: cancel Nav2 goal → `gz service /world/<name>/control` with `reset: {model_only: true}` *and* `gz service /world/<name>/set_pose` with `name: "custom_robot", position: {0,0,0}` (subprocess CLI, interim) — both are required because the dynamically-spawned robot has no "initial pose" registered for WorldControl.reset to restore, so set_pose is what actually teleports it → **slam_toolbox lifecycle reset** (deactivate → cleanup → configure → activate) per ADR-010:62 to start each episode from a fresh pose graph and map → drop subscriber buffers → wait for fresh sensor refresh (5 s timeout) → send new goal. `close()` cancels the active goal so the robot stops when an SB3 trainer or smoke test exits.
- SAC agent — `src/rl_navigation_pkg/rl_navigation_pkg/agents/sac_trainer.py` (training loop with SB3 `SAC` + `CheckpointCallback` + TensorBoard) and `src/rl_navigation_pkg/rl_navigation_pkg/agents/sac_inference.py` (deterministic rollout from a saved checkpoint). Run via `ros2 run rl_navigation_pkg sac_trainer ...` and `ros2 run rl_navigation_pkg sac_inference --checkpoint ...`. Wiring verified 2026-05-11 with a 5000-step training run (8 episodes × 600-step truncate, `learning_starts=500`): fps≈8 (env's 10 Hz minus SAC update overhead), `n_updates` advances 99→4299 monotonically, `critic_loss` 0.031→0.002, `ent_coef` 0.97→0.27, checkpoints saved. **Note**: `ep_rew_mean` stayed flat at −0.216 across all 8 episodes — expected, because the robot is static during training (Nav2 has no goal source from inside the env yet) so reward is dominated by EKF integration drift that does not depend on σ. Real task-learning is blocked on SAC-4 (Nav2 goal driver in env reset/step).
- Smoke-test entries — `src/rl_navigation_pkg/rl_navigation_pkg/nodes/env_smoke_test.py` (v0; 50 random steps), `src/rl_navigation_pkg/rl_navigation_pkg/nodes/v1_smoke_test.py` (v1; 2 episodes × 25 steps with reset between), and `src/rl_navigation_pkg/rl_navigation_pkg/nodes/sac_smoke_test.py` (SAC wiring; 200-step learn run with `learning_starts=50`). Wired as `ros2 run rl_navigation_pkg {env_smoke_test,v1_smoke_test,sac_smoke_test}`.
...
- **SAC-6: slam_toolbox does not survive a second env-driven lifecycle cycle.** SAC-5 made `spawn_robot.launch.py`'s `OnStateTransition(goal_state='inactive')` handler one-shot via an `OpaqueFunction` closure flag, which fixed the racing-activate hypothesis: episode 0's `deactivate→cleanup→configure→activate` now succeeds (`slam_reset_ok=True`, robot drives, reward grows). But the second env-driven reset (episode 1) still fails on the first transition (`DEACTIVATE`). Empirically slam_toolbox seems not to support being cycled through a full cleanup more than once per process lifetime — once we've cleaned+reconfigured it once, the next deactivate is refused. The env's `_slam_lifecycle_transition` now logs slam's `state_before` and `state_after` for each failed transition (added 2026-05-12); the next session should capture that log and choose between (a) replacing the cycle with `clear_changes` (lightweight, preserves the active state), (b) bypassing slam_toolbox entirely and serving Nav2 with `nav2_map_server` over a pre-built map of the fixed training arena (Phase 1 training world never changes — this is the most decisive option), or (c) killing+respawning the slam_toolbox process between episodes via launch.

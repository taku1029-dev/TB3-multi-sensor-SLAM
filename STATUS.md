...
- **Inference-based evaluation of the first training run** — `final.zip` and 10 mid-training checkpoints are saved in `./sac_runs/phase1_001/`; `sac_inference` has not yet been pointed at them. Until that happens we can't tell whether the partial-convergence trajectory observed during training reflects a *usable* policy (deterministic σ choices are coherent, robot survives episodes longer than random) or an *unstable* one (oscillating around a bad local optimum). This is the immediate next action and gates every downstream decision.
- **Convergent Phase 1 policy.** Whether achieved by more training, hyperparameter tuning, or env/reward redesign depends on what the inference rollout above reveals. Likely directions if needed: tighten σ range, add recent-action and velocity to obs, add reward shaping for divergence proximity, or simplify the goal sampler (constant goal during early training, random later).

...
- `SACEnv` Gymnasium env — `src/rl_navigation_pkg/rl_navigation_pkg/envs/sac_env.py`. Id `RLNavigation-v1`, the trained-system env per ADR-005. **A-1 + A-2 + A-3 + A-4 complete** — env-side scaffolding for SAC training is done. Observation: 15-dim `Box(-1, 1)` per ADR-008 (lidar quality 4 + camera quality 2 + IMU dyn 6 + EKF cov 3) with per-feature normalisation per ADR-009:35-48. Action: 2-dim `Box(-1, 1)` per ADR-009 (`σ = exp(a · 3)`). Reward: `-sqrt(dx² + dy²) * 10` per ADR-004:39-40 (subtrahend path verified by `reward_probe`). `step()` runs the per-cycle protocol (ADR-011:26-29 / ADR-012:17-30): `set_parameters` to `/ekf_input_gate` with 50 ms ack deadline → `release` Trigger → 100 ms spin → obs/reward construction. Single `set_parameters` timeouts retry the same action up to 10 times (invisible to SAC per ADR-012:23-30); persistent failure → `truncated=True` with `info['truncated_reason']='set_param_unreachable'`. `/odom` not advancing during the spin → `truncated=True` with `'odom_silent'` (ADR-012:51). Collision (lidar < 0.15 m) and EKF divergence (|EKF − GT| > 2 m) → `terminated=True` per ADR-010:21-26. `reset()` runs the ADR-010:73 sequence: `gz service /world/<name>/control` with `reset: {model_only: true}` (subprocess CLI, interim) → drop subscriber buffers → wait for fresh sensor refresh (5 s timeout). slam_toolbox `clear_map` is intentionally skipped (acceptable in Phase 1 because reward is on EKF pose, not slam_toolbox-corrected pose — ADR-007:46). Verified 2026-05-11: 2-episode × 25-step random-action smoke test runs to natural completion, `set_param_ok` 50/50, σ covers `[0.05, 20]`, all topics refresh after reset, Block D x/yaw varies in [0.07, 0.12] range (y still saturates at COV_MAX=10 — case for SAC to learn, not a bug).
...
- **SAC-4: Nav2 goal driver inside `RLNavigation-v1`** — without something publishing `/navigate_to_pose` goals during training, the robot stays at spawn for the entire episode and reward is flat regardless of σ; SAC has nothing to learn from. ADR-010:72 already specifies the design (uniform random goals within world bounds, rejection sampling for obstacles, mid-episode goal switching when one is reached). Implementation needs: NavigateToPose action client in `_V1Node`, goal sampler in `reset()`, goal-reached / goal-aborted detection to re-sample mid-episode, and a `world_bounds` config knob (currently the 10×10 m arena is hard-coded in `worlds/example_world.sdf`).
...
- Episode-reset path for slam_toolbox (ADR-010:62 `clear_map` service call) — intentionally deferred. Phase 1 reward is on the EKF's filtered pose (ADR-007:46) so a stale slam_toolbox map across episodes does not corrupt the gradient. Revisit when Phase 2 lidar-odom feeds slam_toolbox or when map-frame-based reward becomes relevant.
- ros_gz_bridge **service** bridging — `RLNavigation-v1.reset()` currently shells out to the `gz` CLI for the world-control service. A persistent rclpy client (via `ros_gz_interfaces/srv/ControlWorld` once the launch bridges that service) would remove subprocess overhead from every reset.
